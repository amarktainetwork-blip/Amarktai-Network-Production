...
export default function Dashboard() {
  ...
  const [activeBotTab, setActiveBotTab] = useState('exchange');
  ...
  const handleRealTimeUpdate = (data) => {
    switch (data.type) {
      ...
      case 'force_refresh':
        // FORCE IMMEDIATE REFRESH from AI action
        loadBots();
        loadSystemModes();
        loadMetrics();
        loadProfitData();
  ...
  const confirmLiveSwitch = async (lunoFunded, usePaperBots) => {
    try {
      ...
      toast.error(err.response?.data?.detail || 'Failed to switch to live trading');
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    if (awaitingPassword) {
      if (originalInput === ADMIN_PASSWORD) {
        if (adminAction === 'show') {
          ...
          setChatMessages(prev => [...prev, { role: 'assistant', content: '✅ Admin panel activated!' }]);
        } else if (adminAction === 'hide') {
          setShowAdmin(false);
          sessionStorage.setItem('adminPanelVisible', 'false');
          // Switch to welcome section when hiding admin
          setActiveSection('welcome');
          setTimeout(() => showSection('welcome'), 100);
          setChatMessages(prev => [...prev, { role: 'assistant', content: '🔒 Admin panel hidden. Switched to Welcome section.' }]);
        }
        setAwaitingPassword(false);
        setAdminAction(null);
      } else {
        setChatMessages(prev => [...prev, { role: 'assistant', content: '❌ Incorrect password. Try again.' }]);
        setAwaitingPassword(false);
        setAdminAction(null);
      }
      return;
    ...
    if (msgLower === 'show admin' || msgLower === 'show admn') {
      ...
      setAdminAction('show');
      setChatMessages(prev => [...prev, { role: 'assistant', content: 'Please enter the admin password to show the admin section.' }]);
      return;
    }
    ...
    if (msgLower === 'hide admin') {
      setAwaitingPassword(true);
      setAdminAction('hide');
      setChatMessages(prev => [...prev, { role: 'assistant', content: 'Please enter the admin password to hide the admin section.' }]);
      return;
    }
  ...
  const handleCreateBot = async (e) => {
    ...
    try {
      ...
      const errorMsg = err.response?.data?.detail || 'Failed to create bot';
  ...
  const handleBotSetup = async () => {
    ...
    try {
      ...
      showNotification(`✅ Created ${res.data.created} bots successfully!`, 'success');
      loadBots();
      showSection('bots');
    } catch (err) {
      const errorMsg = err.response?.data?.detail || 'Failed to create bots';
      showNotification(`❌ ${errorMsg}`, 'error');
    }
  };
  ...
  const handleSaveApiKey = async (provider) => {
    ...
    try {
      ...
      const errorMsg = err.response?.data?.detail || 'Failed to save API key';
      showNotification(errorMsg, 'error');
          ...
          <div className="bot-container">
            <div className="bot-left">
              <div className="bot-tabs">
                <button className={`bot-tab ${activeBotTab === 'setup' ? 'active' : ''}`} onClick={() => setActiveBotTab('setup')}>
                  ⚙️ Setup Bots
                </button>
          ...
          <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '12px', marginBottom: '16px'}}>
            ...
            <div style={{
              ...
              <div style={{fontSize: '1.75rem', fontWeight: 700, color: '#f59e0b', marginTop: '6px', display: 'flex', alignItems: 'baseline', justifyContent: 'center', gap: '4px'}}>
                R{profitData?.best_day?.toFixed(2) || '0.00'}
            ...
            <>
                  ...
                  <div style={{marginTop: '20px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', width: '100%'}}>
                    <div style={{textAlign: 'center', padding: '12px', background: 'var(--panel)', borderRadius: '8px'}}>
                      ...
                      <div style={{fontSize: '1.3rem', fontWeight: 700, color: 'var(--text)'}}>
                        R{countdownData.current_capital?.toLocaleString() || '0'}
                      </div>
                    </div>
                    <div style={{textAlign: 'center', padding: '12px', background: 'var(--panel)', borderRadius: '8px'}}>
                  ...
                  <div style={{marginTop: '24px', width: '100%', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px'}}>
                    <div style={{textAlign: 'center', padding: '12px', background: 'var(--glass)', borderRadius: '8px'}}>
                      ...
                      <div style={{fontSize: '1.1rem', fontWeight: 600, color: 'var(--text)'}}>
                        R{countdownData.remaining?.toLocaleString() || '0'}
                      </div>
                    </div>
                    <div style={{textAlign: 'center', padding: '12px', background: 'var(--glass)', borderRadius: '8px'}}>
              ...
              <div style={{marginBottom: '24px'}}>
                <div style={{display: 'flex', justifyContent: 'space-between', marginBottom: '10px', fontSize: '0.9rem', fontWeight: 600}}>
                  <span style={{color: 'var(--text)'}}>R{countdownData.current_capital?.toLocaleString() || '0'}</span>
                  ...
                  <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px'}}>
                    <div style={{textAlign: 'center', padding: '16px', background: 'var(--panel)', borderRadius: '8px'}}>
                      ...
                      <div style={{fontSize: '1.6rem', fontWeight: 700, color: '#3b82f6'}}>
                        R{countdownData.projections?.twelve_month?.toLocaleString() || '0'}
                      </div>
                    </div>
                    <div style={{textAlign: 'center', padding: '16px', background: 'var(--panel)', borderRadius: '8px'}}>
                      ...
                      <div style={{fontSize: '1.6rem', fontWeight: 700, color: 'var(--success)'}}>
                        +R{countdownData.projections?.twelve_month_gain?.toLocaleString() || '0'}
                      </div>
                    </div>
                    <div style={{textAlign: 'center', padding: '16px', background: 'var(--panel)', borderRadius: '8px'}}>
              ...
              <div style={{marginTop: '12px', padding: '12px', background: 'var(--glass)', borderRadius: '6px', border: '1px solid var(--line)', fontSize: '0.85rem', color: 'var(--muted)'}}>
                <p><strong>How it works:</strong> Based on your current capital (R{countdownData.current_capital?.toLocaleString() || '0'}) and daily ROI ({countdownData.metrics?.daily_roi_pct?.toFixed(3) || '0'}%), the system uses compound interest calculations to project your path to R1,000,000. Updates in real-time as you trade!</p>
              </div>
            </>
          )}
        </div>
      ...
      <section className="section active">
        <div className="card">
          ...
          <div style={{marginBottom: '16px', display: 'flex', gap: '12px', flexWrap: 'wrap'}}>
            <div style={{background: 'var(--success)', padding: '8px 16px', borderRadius: '6px', fontWeight: 600}}>
              ZAR: R{balances.zar.toFixed(2)}
            </div>
            <div style={{background: 'var(--success)', padding: '8px 16px', borderRadius: '6px', fontWeight: 600}}>
              BTC: {balances.btc.toFixed(4)} BTC
            </div>
          </div>
          
          <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '12px'}}>
            {/* Deposit Section */}
            <div style={{display: 'flex', flexDirection: 'column', gap: '10px', border: '1px solid var(--line)', borderRadius: '6px', padding: '16px', background: 'var(--panel)'}}>
              <h3>💵 Deposit Funds</h3>
              <p style={{fontSize: '0.85rem', color: 'var(--muted)'}}>Send ZAR or BTC to these addresses:</p>
              
              <div style={{marginTop: '8px'}}>
                <label style={{fontSize: '0.85rem', color: 'var(--muted)', fontWeight: 600}}>ZAR Deposit Address</label>
                <div style={{display: 'flex', gap: '6px', marginTop: '4px'}}>
                  <input 
                    type="text" 
                    value={depositAddress?.zar || 'Loading...'} 
                    readOnly
                    style={{flex: 1, padding: '8px', borderRadius: '6px', border: '1px solid var(--line)', background: 'var(--glass)', color: 'var(--text)', fontSize: '0.85rem'}} 
                  />
                  <button 
                    onClick={() => copyAddress(depositAddress?.zar)}
                    style={{padding: '8px 12px', borderRadius: '6px', background: 'var(--accent2)', color: 'var(--text)', border: 'none', fontWeight: 600, cursor: 'pointer'}}
                  >
                    Copy
                  </button>
                </div>
              ...
              <div style={{marginTop: '12px'}}>
                <label style={{fontSize: '0.85rem', color: 'var(--muted)', fontWeight: 600}}>BTC Deposit Address</label>
                <div style={{display: 'flex', gap: '6px', marginTop: '4px'}}>
                  <input 
                    type="text" 
                    value={depositAddress?.btc || 'Loading...'} 
                    readOnly
                    style={{flex: 1, padding: '8px', borderRadius: '6px', border: '1px solid var(--line)', background: 'var(--glass)', color: 'var(--text)', fontSize: '0.85rem'}} 
                  />
                  <button 
                    onClick={() => copyAddress(depositAddress?.btc)}
                    style={{padding: '8px 12px', borderRadius: '6px', background: 'var(--accent2)', color: 'var(--text)', border: 'none', fontWeight: 600, cursor: 'pointer'}}
                  >
                    Copy
                  </button>
              ...
              <div style={{marginTop: '12px', padding: '12px', background: 'var(--glass)', borderRadius: '6px', fontSize: '0.85rem', color: 'var(--muted)'}}>
                ⚠️ <strong>Important:</strong> Only send ZAR/BTC to the respective addresses. Deposits typically confirm within 10-30 minutes.
              </div>
            </div>
            ...
            <div style={{display: 'flex', flexDirection: 'column', gap: '10px', border: '1px solid var(--line)', borderRadius: '6px', padding: '16px', background: 'var(--panel)'}}>
              ...
              <label style={{fontSize: '0.85rem', color: 'var(--muted)'}}>Withdraw Amount (ZAR)</label>
              <input type="number" placeholder="Amount (ZAR)" style={{width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid var(--line)', background: 'var(--glass)', color: 'var(--text)'}} />
              <label style={{fontSize: '0.85rem', color: 'var(--muted)'}}>Withdrawal Address</label>
              <input type="text" placeholder="Your Bank Account or Crypto Address" style={{width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid var(--line)', background: 'var(--glass)', color: 'var(--text)'}} />
              <button style={{padding: '8px 12px', borderRadius: '6px', background: 'var(--error)', color: 'var(--text)', border: 'none', fontWeight: 600, cursor: 'pointer'}} onClick={() => showNotification('Withdrawal request sent')}>Send Withdrawal</button>
            </div>
            
            {/* Tradable Funds */}
            <div style={{display: 'flex', flexDirection: 'column', gap: '10px', border: '1px solid var(--line)', borderRadius: '6px', padding: '16px', background: 'var(--panel)'}}>
              <h3>💰 Tradable Funds Setup</h3>
              <label style={{fontSize: '0.85rem', color: 'var(--muted)'}}>Max % of ZAR for bots</label>
              <input type="number" min="0" max="100" defaultValue="100" style={{width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid var(--line)', background: 'var(--glass)', color: 'var(--text)'}} />
              <label style={{fontSize: '0.85rem', color: 'var(--muted)'}}>Max % of BTC for bots</label>
              <input type="number" min="0" max="100" defaultValue="100" style={{width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid var(--line)', background: 'var(--glass)', color: 'var(--text)'}} />
              <button style={{padding: '8px 12px', borderRadius: '6px', background: 'var(--accent2)', color: 'var(--text)', border: 'none', fontWeight: 600, cursor: 'pointer'}} onClick={() => showNotification('Fund rules saved')}>Save Fund Rules</button>
            </div>
          </div>
        </div>
      </section>
                    ...
                    {flokxAlerts.map((alert, idx) => (
                      ...
                      >
                        <div>
                          ...
                          <div style={{fontSize: '0.85rem', color: 'var(--muted)'}}>{alert.message}</div>
                        ...
                        <div style={{
                          ...
                          fontSize: '0.75rem',
                          fontWeight: 600,
                          background: getAlertColor(alert.priority),
                          color: 'white'
                        }}>
    ...
    <div className="app">
      ...
      {!isMobile && (
        <aside className="sidebar">
          <img
            src={`https://res.cloudinary.com/dns1nzdhx/image/upload/v1758957587/logo1_jphlyu.png?t=${Date.now()}`}
            className="logo"
            alt="Logo"
          ...
          <nav className="nav">
            ...
            <a href="#" className={activeSection === 'wallet' ? 'active' : ''} onClick={(e) => { e.preventDefault(); showSection('wallet'); }}>💰 Luno Wallet</a>
      ...
      <main className="main">
        ...
        {activeSection === 'wallet' && renderLunoWallet()}
